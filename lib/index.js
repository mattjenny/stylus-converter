...
function visitProperty(node) {
  ...
  if (segmentsText === 'absolute' || segmentsText === 'relative' || segmentsText === 'fixed') {
    var transformed = before + 'position: ' + segmentsText + ';';
...
function visitIdent(_ref2) {
  ...
  if (identVal.__type === 'Expression') {
    ...
    // })
    var expText = visitExpression(identVal);
...
var KNOWN_FUNCTIONS = ['rgb', 'rgba', 'var', 'calc', 'scale', 'rotate', 'translate', 'linear-gradient', 'rect', 'grayscale', 'translate3d', 'translateX', 'translateY', 'translateZ', 'url', 'format', 'local', 'repeat'];
...
function visitCall(_ref3) {
  ...
  if (MIXINS[name]) {
    ...
    var argTokens = argsText.split(',');
    var separator = name === 'darken' ? ', $lightness: -' : ', $lightness: ';
    ...
    var _argTokens = argsText.split(',');
    var color2 = name === 'blend' ? _argTokens[1].trim() || '#fff' : name === 'mix' ? _argTokens[1].trim() : name === 'shade' ? '#000000' : '#ffffff';
    var amount = name === 'blend' ? '$weight: 50%' : name === 'mix' ? _argTokens[2] : _argTokens[1];
    usePaths['sass:color'] = true;
    return before + 'color.mix(' + _argTokens[0] + ', ' + color2 + ', ' + amount + (')' + blockText + ';');
